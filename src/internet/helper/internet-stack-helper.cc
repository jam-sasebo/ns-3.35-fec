...
void
InternetStackHelper::InstallwithFec (Ptr<Node> node, bool codeType) const
{
  if (m_ipv4Enabled)
    {
      if (node->GetObject<Ipv4> () != 0)
      ...
      if (codeType == 0) 
        {
           CreateAndAggregateObjectFromTypeId (node, "ns3::Ipv4L3ProtocolwithFecEncode");
        }
      else if (codeType == 1)
        {
           CreateAndAggregateObjectFromTypeId (node, "ns3::Ipv4L3ProtocolwithFecDecode");
        }
      CreateAndAggregateObjectFromTypeId (node, "ns3::Icmpv4L4Protocol");
      if (m_ipv4ArpJitterEnabled == false)
        {
