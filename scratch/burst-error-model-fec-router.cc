...
printUsage()
{
  std::cout
  << "---------------------------------------------\n"
  << "Usage: --nRouters: Number of routers\n"
  << "\t--burstRate: Error Rate of Burst Error Model(0.0001)\n"
  << "\t--burstSize: Burst Size of the error model, i.e. number \nof packets to drop in a burst\n"
  << "---------------------------------------------\n";
}
///*
void
  ...
  for (uint32_t i = 0; i < nRouters-2; i++)
    {
      ...
      R1_RkInterfaces.push_back (r_rinterfaces);
      address.NewNetwork ();
    }
