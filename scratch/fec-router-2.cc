...
int main (int argc, char *argv[])
{
  ...
  */
  ...
  if (fecmode)                                                                          
    {                                                                                  
     internetStack.InstallwithFec (routersNodes.Get (0), 0); //encode        
     internetStack.InstallwithFec (routersNodes.Get (1), 1); //decode                      
     FecRouterAgentHelper FecRouterAgent;
     FecRouterAgent.Create (1);
     FecRouterAgent.Set (0, routersNodes.Get(0),0);
     FecRouterAgent.Set (0, routersNodes.Get(1),1);
    }                                                                                
  else                                                                                  
    {                                                                                  
 ...
 FecAgent::EnableTcpDump (ascii.CreateFileStream ("FecRouter-2.TcpDump"));
 FecAgent::EnablePrintTimeout (ascii.CreateFileStream ("FecRouter-2.Timeout"));
 FecAgent::EnablePrintFastRecovery (ascii.CreateFileStream ("FecRouter-2.FastRecovery"));
 FecAgent::EnablePrintRetransmit (ascii.CreateFileStream ("FecRouter-2.Retransmit"));

 FecRouterAgent::EnablePrintRoundFecTable (ascii.CreateFileStream ("FecRouter-2.RoundFecTable"));
 FecRouterAgent::EnablePrintGroupFecTable (ascii.CreateFileStream ("FecRouter-2.GroupFecTable"));
 FecRouterAgent::EnablePrintRecovery (ascii.CreateFileStream ("FecRouter-2.Recovery"));
 FecRouterAgent::EnablePrintLostPacket (ascii.CreateFileStream ("FecRouter-2.LostPacket"));
 FecRouterAgent::EnablePrintSentPacket (ascii.CreateFileStream ("FecRouter-2.SentPacket"));
 

 char str[100];
 char str1[]="fec-router-2-cwnd";
 char str3[]=".dat";
